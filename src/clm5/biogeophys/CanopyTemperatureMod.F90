      ...
      do fc = 1,num_nolakec
         ...
         if (lun%itype(l)/=istwet .AND. lun%itype(l)/=istice_mec) then
            ...
            if (lun%itype(l) == istsoil .or. lun%itype(l) == istcrop) then
               ...
               if (pfl_psi(c,1) < 0.0_r8)  psit = max(smpmin(c), pfl_psi(c,1))
!               if (pfl_psi(c,1) < 0.0_r8)  psit = max(-10._r8, pfl_psi(c,1)) ! nan to cpl7, lines could be rewritten to w min/max
!               statement
#else
