module atm_comp_mct
  ...
  !use icon_comp_mod   , only: icon_comp_init, icon_comp_run, icon_comp_final
...
CONTAINS
  ...
  subroutine atm_init_mct( EClock, cdata, x2a, a2x, NLFilename )
    ...
    integer           :: phase                     ! initialization phase index
    ...
    !call icon_comp_init(Eclock, x2a, a2x, &
    !     seq_flds_x2a_fields, seq_flds_a2x_fields, &
    !     mpicom, compid, my_task, master_task, &
    !     inst_suffix, inst_name, logunit, phase, nextsw_cday)
    ...
    if (phase == 1) then
       call seq_infodata_PutData(infodata, &
            atm_nx=100, &   !SDATM%nxg
            atm_ny=100, &   !SDATM%nyg
            atm_aero=.false., &
    ...
    if (my_task == master_task) write(logunit,F00) 'icon_comp_init done'
...
!          case_name = case_name)
  ...
  subroutine atm_final_mct(EClock, cdata, x2a, a2x)
    ...
    ! call  icon_comp_final(my_task, master_task, logunit)
